...
public:
    ...
    Compiler(Symtab* symtab) : baseSymtab(symtab)
    {
        stack = new FrameStack(&out);
        stack->push(baseSymtab, 1);
        addRequiredCode();
    }

    string getOutput() {return out;}
    ...
    void addRequiredCode() {
        
        addInitFrame();
        addMemManagement();
        out += "END exit\n"; // Exit label at end of program
    }
